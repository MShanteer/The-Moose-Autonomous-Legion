---
name: coding-legion
description: Parallel agent swarm for the WeatherOps repo — shard a Codex (Brain) plan into dependency-ordered lanes and execute them with parallel Claude subagents under LoopX leases, then muscle-review the merged diff. Use when the owner says "coding legion", "swarm this", "parallelize", "run lanes", or a Brain plan has 3+ independent increments. Composes Codex→Claude→LoopX→Swarm; never replaces the Brain.
---

# 🦌 Coding Legion — parallel lanes under the Brain & Muscle model (WeatherOps)
...
- LoopX goal is bootstrapped (`.loopx/registry.json`; Legion goal:
  `weathercrm-ui-ux-mobile-polish` unless a new goal fits better).
...
- **Cheap mechanical lanes may dispatch to Moose Code Legion instead**
  (`/c/dev/moose-code-legion/scripts/mcl "WHOLE TASK IN ONE STRING"`) — MCL has
  no memory between calls, so its context pack must be fully self-contained
  and its output ALWAYS verified by the orchestrator before the lane closes.
- Each subagent prompt is a **context pack**: its task block verbatim, the
  ...
  `docs/WEATHEROPS_CONTEXT_BRIEF.md`), and the completion contract below.
...
(staging deploy per docs/deploy-runbook.md) + targeted live checks on localhost — cross-lane breakage is
...
- Secrets never enter the plan file or lane prompts — lanes read env
  themselves.
- WeatherOps-only: do not blend with another repo's brain/muscle/LoopX. The
