---
name: coding-legion
description: Parallel agent swarm for the WeatherOps repo — shard a Codex (Brain) plan into dependency-ordered lanes and execute them with parallel Claude subagents under file-disjoint lane claims, then muscle-review the merged diff. Use when the owner says "coding legion", "swarm this", "parallelize", "run lanes", or a Brain plan has 3+ independent increments. Composes Codex→Claude→Swarm; never replaces the Brain.
---
...
- Secrets never enter the plan file or lane prompts — lanes read env
  themselves.
- WeatherOps-only: do not blend with another repo's brain/muscle/plan files. The
